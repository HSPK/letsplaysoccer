/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: Saturday, July 11, 2020 AM09:27:40 CST
 ************************************************************************/
...
void show_message(WINDOW *win, struct ChatMsg *msg, int type) {
        ...
        char buff[512] = {0};
    if (!(msg->type & CHAT_FUNC)) {
        sprintf(buff, "<%s> : ", msg->from);
        w_gotoxy_puts(win, 10, msgnum, buff);
    }
    ...
    for (int i = 0; i < strlen(msg->msg); i++) {
        ...
        char num[512] = {0};
        //msgnum = msgnum + tmp + nnl - 1;
        msgnum += tmp;
        //sprintf(num, "%s %d %d %d", msg->msg, msgnum, tmp, nnl);
    //static int ninfo = 1;
      //  w_gotoxy_puts(info_win, 1, ninfo++, num);
        //wrefresh(info_win);
        wrefresh(win);
        wmove(input_win, 2, 1);
        wrefresh(input_win);
